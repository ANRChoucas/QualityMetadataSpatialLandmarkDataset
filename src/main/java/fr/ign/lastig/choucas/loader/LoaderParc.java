...
public class LoaderParc {
	
	public static List<Feature> getToponyme() {
		
		List<Feature> parcFeature = new ArrayList<Feature>();
		
		try {
			
			File fileC2C = new File("./data/dataset/Dataset_RandoEcrinsParcnational_RandoParcDuVercors.csv");
    		...
    		try (CsvReader csv = CsvReader.builder()
				...
				for (final Iterator<CsvRow> iterator = csv.iterator(); iterator.hasNext();) {
	    			...
	    			parcFeature.add(defaultFeature);
				}
    		}
